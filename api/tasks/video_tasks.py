...
def _locate_peak_window(
    ...
    try:
        ...
        while True:
            ...
            if w > 320:
                frame = cv2.resize(
                    frame, (320, int(h * 320 / w)), interpolation=cv2.INTER_AREA
                )
...
def process_clip(self, clip_id: str) -> dict:
    ...
    try:
        ...
        if result.is_anomaly:
            type_str, ambiguous_type = _anomaly_type_from_zscores(
            ...
            top_zscores = dict(
                sorted(
                    result.feature_zscores.items(), key=lambda x: x[1], reverse=True
                )[:5]
            )
                ...
                Anomaly(
                    ...
                    detection_metadata={
                        "top_deviant_features": {
                            k: round(v, 3) for k, v in top_zscores.items()
                        },
        ...
        for row in anomaly_rows:
            row.score_impact = round(
                score_result.deduction_breakdown.get(row.anomaly_type.value, 0.0), 2
            )
        ...
        if anomaly_rows:
            try:
                ...
                logger.warning(
                    "[%s] AI explanation failed: %s", clip.filename_prefix, exc
                )
        ...
        try:
            ...
            logger.warning(
                "[%s] Overall score recalc failed: %s", clip.filename_prefix, exc
            )
