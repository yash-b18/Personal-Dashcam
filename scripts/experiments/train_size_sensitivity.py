...
def _load_split_features() -> dict:
    ...
    logger.info("Train: %d (%d pos)  Test: %d (%d pos)",
                len(y_tr), y_tr.sum(), len(y_te), y_te.sum())
    return {
        "X_train": X_tr, "y_train": y_tr, "train_ids": train_ids,
        "X_test": X_te, "y_test": y_te, "test_ids": test_ids,
    }
...
def _train_classical(X_tr, y_tr, X_te, y_te, seed: int) -> dict:
    ...
    return {
        ...
        "auc":       float(roc_auc_score(y_te, probs)) if len(set(y_te)) > 1 else float("nan"),
    ...
    for frac in TRAIN_FRACTIONS:
        for seed in SEEDS:
            X_sub, y_sub = _stratified_subsample(
                data["X_train"], data["y_train"], frac, seed,
            )
            ...
            logger.info(
                ...
                frac, seed, m["n_train"], m["n_train_pos"], m["f1"], m["auc"],
            )
            runs.append(m)
    return runs
...
def _load_lstm_data(train_ids: list[str], test_ids: list[str],
    ...
    def _load(ids):
        ...
        if not X_windows:
            return None, None, []
        return (np.concatenate(X_windows, axis=0),
                np.concatenate(y_windows, axis=0),
                clip_of_window)

    X_tr, y_tr, tr_clip = _load(train_ids)
    X_te, y_te, te_clip = _load(test_ids)
    logger.info("LSTM train windows: %d (%d pos)  test windows: %d (%d pos)",
                len(y_tr), int(y_tr.sum()), len(y_te), int(y_te.sum()))
    return {
        "X_train": X_tr, "y_train": y_tr, "train_clip": tr_clip,
        "X_test":  X_te, "y_test":  y_te, "test_clip":  te_clip,
    }
...
def _train_lstm(X_tr, y_tr, X_te, y_te, test_clip, seed: int, epochs: int = 10) -> dict:
    ...
    return {
        ...
        "auc":       float(roc_auc_score(y_true, p_arr)) if len(set(y_true)) > 1 else float("nan"),
...
def run_lstm_sensitivity(train_ids, test_ids, labels, seeds=(0, 1)) -> list[dict]:
    ...
    clip_label = {c: int(data["y_train"][data["train_clip"].index(c)])
                  for c in clip_list}
    ...
    for frac in TRAIN_FRACTIONS:
        for seed in seeds:
            ...
            keep = set(rng.choice(pos_clips, size=n_pos, replace=False)) | \
                   set(rng.choice(neg_clips, size=n_neg, replace=False))
            mask = np.array([c in keep for c in data["train_clip"]])
            X_sub = data["X_train"][mask]
            y_sub = data["y_train"][mask]
            m = _train_lstm(X_sub, y_sub,
                            data["X_test"], data["y_test"], data["test_clip"],
                            seed=seed)
            m.update(fraction=frac, seed=seed, model="lstm",
                     n_train_clips=len(keep))
            logger.info(
                "lstm frac=%.2f seed=%d n_clips=%d pos_clips=%d f1=%.3f auc=%.3f",
                frac, seed, len(keep), n_pos, m["f1"], m["auc"],
            )
            runs.append(m)
    return runs
...
def _plot(runs: list[dict], out_path: Path) -> None:
    ...
    for ax, metric, title in [(axes[0], "f1", "F1 on held-out test"),
                              (axes[1], "auc", "AUC on held-out test")]:
        for m in models:
            rows = [r for r in runs if r["model"] == m]
            fracs = sorted({r["fraction"] for r in rows})
            means = [np.nanmean([r[metric] for r in rows if r["fraction"] == f])
                     for f in fracs]
            stds = [np.nanstd([r[metric] for r in rows if r["fraction"] == f])
                    for f in fracs]
...
def main() -> None:
    ...
    ap.add_argument("--include-lstm", action="store_true",
