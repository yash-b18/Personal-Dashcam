...
class TestDetectWindows:
    ...
    def test_window_contains_spike_frame(self, detector: OpticalFlowBaseline) -> None:
        ...
        windows = detector._detect_windows(magnitudes)
        assert any(w["start_frame"] <= spike_frame < w["end_frame"] for w in windows)
...
class TestSeverity:
    ...
    def test_severity_monotonically_increases(
        self, detector: OpticalFlowBaseline
    ) -> None:
        ...
        raw = [
            {
                "start_frame": 0,
                "end_frame": 30,
                "peak_magnitude": MAGNITUDE_THRESHOLD + 10,
                "mean_magnitude": 15.0,
                "magnitude_std": 3.0,
            }
        ]
